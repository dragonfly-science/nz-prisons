...
prison_pop %>%
    group_by(Year, Sex, Age, Ethnicity, Offence, Duration) %>%
    count() %>%
    .$n %>%
    unique()

prison_pop %>%
    mutate(Population_Count = case_when(
               is.na(Population_Count) ~ Estimated_Count,
               TRUE ~ Population_Count)) %>%
    View()
...
prison_vars <- c("Year", "Sex", "Age", "Ethnicity", "Offence", "Duration", "Prisoner_Count", "Population_Count")
