...
functions {
  /* Random rounding to base n log-PDF of a single response
   * Args:
   *   y: the rounded count
   *   yhat: the unrounded count
   *   base: the base of the random rounding procedure
   * Returns:
   *   a randomly rounded response
   */
...
parameters {
  ...
  real<lower=0,upper=1> p_rr; // random rounding noise
}
...
model {
  ...
  if (!prior_only) {
    for (n in 1:N) {
      ...
      target += random_rounding_lpmf(Y[n] | 3);
    }
  }
}
